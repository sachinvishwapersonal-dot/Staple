  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="flex items-center justify-between border-b border-border px-6 py-6 md:px-12">
        ...
        <Link
          to="/blog"
          className="text-[11px] font-bold uppercase tracking-[0.2em] text-warmgrey transition-colors hover:text-foreground"
        >
          Knowledge base
        </Link>
      </header>
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-3xl flex-col items-start justify-between gap-4 px-6 py-10 md:flex-row md:items-center">
          ...
          <Link to="/blog" className="text-xs font-medium text-warmgrey hover:text-foreground">
            All notes
          </Link>
        </div>
      </footer>
  ...
  return (
    <Chrome>
      <section className="mx-auto max-w-3xl px-6 py-24">
        <h1 className="text-3xl font-extrabold tracking-tight">This note is unavailable.</h1>
...
function BlogPostPage() {
  ...
  return (
    <Chrome>
      <article className="mx-auto max-w-3xl px-6 pb-20 pt-16 md:pt-24">
        ...
        <p className="mt-6 text-lg font-light leading-relaxed text-foreground">{post.excerpt}</p>
        <div className="mt-12 space-y-6 border-t border-border pt-10">
          {post.body.map((para) => (
            <p key={para.slice(0, 32)} className="text-base leading-[1.85] text-warmgrey">
              {para}
            </p>
          ))}
        </div>
            ...
            {others.map((other) => (
              <li key={other.slug}>
                <Link
                  ...
                  className="block bg-background p-6 transition-colors hover:bg-secondary/70"
                >
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-warmgrey">
                    {other.category}
                  </p>
                  <p className="mt-2 text-base font-bold tracking-tight">{other.title}</p>
                </Link>
              </li>
            ))}
          </ul>
